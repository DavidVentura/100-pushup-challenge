...
export const ProgressGrid = ({ 
  ...
  const examDayMap = Object.keys(EXAM_CONFIGS).reduce((acc, examDay) => {
  ...
  const renderDay = (day: number) => {
    ...
    const plannedPushups = result ? calculatePushupSets(day, result).reduce((a, b) => a + b, 0) : null;
    return (
      <div 
        key={day}
        className={`p-4 rounded ${
          day === currentDay ? 'border-2 border-blue-500' :
          latestAttempt?.success ? 'bg-green-100' :
          latestAttempt?.success === false ? 'bg-red-100' :
          'bg-gray-50'
        }`}
      >
        <div className="text-sm text-gray-600 mb-1">Day {day}</div>
        <div className="flex items-center justify-between">
          <span className={`${plannedPushups ? "text-gray-900" : "text-gray-300"} font-medium`}>
            {latestAttempt ? latestAttempt.totalPushups : plannedPushups || "N/A"}
          </span>
          {latestAttempt?.success === true && <CheckCircle2 className="w-4 h-4 text-green-500" />}
          {latestAttempt?.success === false && <XCircle className="w-4 h-4 text-red-500" />}
        </div>
      </div>
    );
  };
            ...
            <div className="bg-white px-4 py-2 text-sm font-medium text-gray-500">
              ...
              {examProgress && (
                <span className="ml-2 text-purple-600">
                  ({examProgress.result.pushupRange} pushups - {examProgress.result.level})
                </span>
              )}
            </div>
          </div>
      ...
      elements.push(
        <div key={`grid-${currentDayNum}`} className="grid grid-cols-3 gap-4">
          {[0, 1, 2].map(offset => {
            ...
            return dayNum <= totalDays ? renderDay(dayNum) : <div key={`empty-${dayNum}`} />;
          })}
        </div>
      );
